...
get_num_taxa = function(treefile) {
  phy = read.tree(treefile)
  tips = phy$tip.label
  n_taxa = length(tips)
  return(n_taxa)
...
for (sim in 201:400) {
  ...
  if ((file %in% files) & (treefile %in% treefiles)) {
    ...
    tree_size_paleo <- c(tree_size_paleo, get_num_taxa(treefile))
    sims <- c(sims, sim)
  }
}
for (sim in 1:200) {
  ...
  if ((file %in% files) & (treefile %in% treefiles)) {
    ...
    tree_size_nopaleo <- c(tree_size_nopaleo, get_num_taxa(treefile))
    sims <- c(sims, sim)
  }
}

paleo_df <- cbind(colonization_ages_paleo, crown_ages_paleo, tree_size_paleo, rep("paleo",length(colonization_ages_paleo)))
nopaleo_df <- cbind(colonization_ages_nopaleo, crown_ages_nopaleo, tree_size_nopaleo, rep("nopaleo",length(colonization_ages_nopaleo)))

df <- rbind(paleo_df,nopaleo_df)
df <- cbind(sims,df)
df <- data.frame(df)
colnames(df) <- c("sim","col_age","crown_age","tree_size","geo")
...
for (lbl in c("col_age","crown_age","diff_age")) {
    ...
    plot_fn = paste0("../experiment1/figures/",lbl, ".pdf")
    pdf(plot_fn, height=7, width=7)
    print(p)
    dev.off()
}

p2 = ggplot(df, aes_string(x="tree_size",fill="geo")) +
  geom_density(bw=1.25,alpha=0.6) +
  scale_x_continuous(expand=c(0,0)) +
  scale_y_continuous(limits=c(0,0.15), expand=c(0,0)) +
  scale_fill_manual(labels=c("No Paleo", "Paleo"),
                    values=c("salmon", "cyan3")) +
  labs(x=paste0(lbl, " (# of tips)"),
       y="Density",
       fill="Geography") +
  theme_bw() +
  theme(aspect.ratio=1/2,
...
plot_fn = paste0("../experiment1/figures/","tree_size", ".pdf")
pdf(plot_fn, height=7, width=7)
print(p2)
dev.off()
